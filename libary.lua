...
function Library:save_flags()
    ...
    local success, result = pcall(function()
        ...
        writefile(`Nemesis"/{game.GameId}.lua`, flags)
    end)
end

function Library:load_flags()
    local success, result = pcall(function()
        if not isfile(`Nemesis"/{game.GameId}.lua`) then
            Library.save_flags()
            return
        end

        local flags = readfile(`Nemesis"/{game.GameId}.lua`)
        if not flags then
            Library.save_flags()
            return
        end
...
function Library.__init()
	...
	local Logo = Instance.new("ImageLabel")
    	...
    	Logo.Image = "rbxassetid://93608824694543"
